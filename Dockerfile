...
# NOTE: Chroma catalog embeddings are NOT built at image-build time.
# services/catalog_agent.py self-heals on first startup using the runtime
# GEMINI_API_KEY env var (background thread, ~3-5 min one-time per deploy).
# Until the thread finishes, requests fall back to the legacy keyword path.
