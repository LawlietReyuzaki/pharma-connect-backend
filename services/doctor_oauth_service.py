...
class DoctorOAuthService:
    def __init__(self):
        ...
        if self.has_credentials:
            logging.info("Doctor OAuth service configured with Google credentials")
        else:
            logging.warning("Google OAuth credentials not found for doctor integration")
    ...
    def get_authorization_url(self, doctor_id):
        ...
        try:
            ...
            logging.error(f"Failed to generate OAuth URL for doctor {doctor_id}: {e}")
            return None, None
    ...
    def exchange_code_for_tokens(self, auth_code, state):
        ...
        try:
            ...
            if doctor:
                ...
                doctor.google_email = user_info.get('email') if user_info else None
                doctor.google_connected_at = datetime.utcnow()
                
                db.session.commit()
                
                logging.info(f"✅ Doctor {doctor.name} connected Google Calendar: {doctor.google_email}")
                return {
                    'doctor_id': doctor_id,
        ...
        try:
            ...
            if credentials.expired and credentials.refresh_token:
                try:
                    ...
                    logging.info(f"🔄 Refreshed OAuth token for doctor {doctor.name}")
                except Exception as e:
                    logging.error(f"Failed to refresh token for doctor {doctor.name}: {e}")
                    return None
    ...
    def create_calendar_event_for_doctor(self, doctor_id, appointment_data):
        ...
        try:
            ...
            if not credentials:
                logging.warning(f"No valid Google credentials for doctor {doctor_id}")
                return None
            ...
            event = {
                'summary': appointment_data.get('summary', 'Red Dot Pharmacy - Patient Consultation'),
            ...
            if doctor:
                if meet_link:
                    logging.info(f"✅ SUCCESS: Created REAL Google Meet in {doctor.name}'s calendar: {meet_link}")
                else:
                    logging.error(f"❌ FAILED: Calendar event created in {doctor.name}'s calendar but no Meet link found")
            ...
            logging.error(f"Google Calendar API error for doctor {doctor_id}: {e}")
            return None
        except Exception as e:
            logging.error(f"Failed to create calendar event for doctor {doctor_id}: {e}")
            return None
    ...
    def revoke_doctor_authorization(self, doctor_id):
        ...
        try:
            ...
            if doctor:
                ...
                logging.info(f"🔓 Revoked Google Calendar access for doctor {doctor.name}")
                return True
            return False
            
        except Exception as e:
            logging.error(f"Failed to revoke authorization for doctor {doctor_id}: {e}")
            return False
