...
def chat():
    ...
    try:
        ...
        # Generate response
        response_data = generate_response(text=message,
                                          prefer_urdu=prefer_urdu,
                                          session_id=session_id)
        # print(response_data)

        # Log the interaction
        log_chat_interaction(session_id=session_id,
        ...
        return jsonify({
            ...
            "flagged":
            response_data['flagged'],
            "needs_doctor":
            response_data['needs_doctor'],
            "suggested_medicines":
            response_data['suggested_medicines']
        })
...
def voice_chat():
    ...
    try:
        ...
        response_data = generate_response(text=transcript,
                                          prefer_urdu=prefer_urdu,
                                          session_id=session_id)
        ...
        return jsonify({
            "error":
            "Voice chat is temporarily unavailable. Please try text chat.",
            "success": False
        }), 500
    ...
    try:
        ...
        for session in sessions:
            ...
            first_log = ChatLog.query.filter_by(
                user_id=user.id, session_id=session.session_id).first()
            ...
            session_list.append({
                ...
                "preview": (first_log.message[:50] + "...") if
                (first_log and first_log.message
                 and len(first_log.message) > 50) else
                (first_log.message if first_log and first_log.message else
